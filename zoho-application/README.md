...
1. **Prerequisites**: Ensure the **Zoho Service** is running in development mode with the development `ZOHO_CRM_SOID` configured.
...
1. **Prerequisites**: Ensure the **Zoho Service** is running in production mode with the production `ZOHO_CRM_SOID` configured.
...
- **Development**: If running this service in development, the Zoho Service must also be running in development with the development `ZOHO_CRM_SOID`.
- **Production**: If running this service in production, the Zoho Service must also be running in production with the production `ZOHO_CRM_SOID`.

The `MS_ZOHO_URL` environment variable should point to the Zoho Service instance running in the same environment. Mismatched environments can lead to data corruption or connecting to the wrong Zoho CRM organization.
